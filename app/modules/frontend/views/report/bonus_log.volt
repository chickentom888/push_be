    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="page-title-box">
                <h4 class="page-title">Bonus Log</h4>
            </div>
        </div>
    </div>
        ...
        <div class="col-md-12 col-xl-12 col-sm-12">
            <div class="card bg-white m-b-30">
                <div class="card-body new-user">
                    ...
                    <h5 class="header-title mb-4 mt-0">List Bonus Log</h5>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div>Total: <b>{{ helper.numberFormat(summaryData['bonus_amount'], 2) }}</b></div>
                        </div>
                    </div>
                        ...
                        <table class="table table-hover mb-0">
                            ...
                            {% for item in listData %}
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div>From Amount: <b class="text-info">{{ item['type'] == 'team_bonus' ? helper.numberFormat(item['residual_amount'], 2) : helper.numberFormat(item['from_amount'], 2) }}</b></div>
                                        <div>Amount: <b class="text-success">{{ helper.numberFormat(item['bonus_amount'], 2) }}</b></div>
                                        <div>Percent: <b>{{ item['bonus_percent'] }}%</b></div>
                                    </td>
                                    <td>
                                        {% if item['type'] == 'team_bonus' %}
                                            <div>Before: <b class="text-warning">{{ helper.numberFormat(item['before_save_branch'], 2) }}</b></div>
                                            <div>Last: <b class="text-info">{{ helper.numberFormat(item['last_save_branch'], 2) }}</b></div>
                                            <div>Diff: <b class="text-success">{{ helper.numberFormat(item['residual_amount'], 2) }}</b></div>
                                            <div>Left: <b class="text-dark">{{ helper.numberFormat(item['left_invest'], 2) }}</b></div>
                                            <div>Right: <b class="text-success">{{ helper.numberFormat(item['right_invest'], 2) }}</b></div>
                                            <div>Left F1: <b class="text-dark">{{ helper.numberFormat(item['count_left_f1_invest'], 2) }}</b></div>
                                            <div>Right F1: <b class="text-success">{{ helper.numberFormat(item['count_right_f1_invest'], 2) }}</b></div>
                                        {% endif %}
