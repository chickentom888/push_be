<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-sm-12">
            <div class="page-title-box">
                <h4 class="page-title">Presale Setting </h4>
            </div>
        </div>
    </div>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            {% for key,item in listPlatform %}
                                <a href="/registry/presale_setting/{{ key }}" class="btn btn-{{ platform == key ? 'info' : 'outline-dark' }}">{{ item }}</a>
                            {% endfor %}
                        </div>
                    </div>
                </div>
        ...
        <div class="col-md-12 col-xl-12 col-sm-12">
            ...
            <div class="card bg-white m-b-30">
                ...
                <div class="card-body">
                    ...
                    <h5 class="header-title mb-4 mt-0">Presale Setting Address</h5>

                    <div class="row">
                        <div class="col-sm-6">
                            <div class="form-group">
                                <label for=presale-setting-address>Presale setting address</label>
                                <a id="presale-setting-address" href="{{ helper.getLinkAddress(presaleSettingAddress, platform, network ) }}" target="_blank" class="form-control">{{ presaleSettingAddress }}</a>
                            </div>
                        </div>
                    </div>
    ...
    {% include 'partials/presale/presale_setting_base_token.volt' %}

    {% include 'partials/presale/presale_setting_main_info.volt' %}

    {% include 'partials/presale/presale_setting_address_info.volt' %}

    {% include 'partials/presale/presale_setting_whitelist_token.volt' %}

    {% include 'partials/presale/presale_setting_zero_round.volt' %}
    <!-- end row -->
...
<script type="text/javascript">
    ...
    async function deleteBaseToken(obj) {
        try {
            ...
            let result = await presaleSettingContract.methods.updateBaseToken(baseTokenContract, false).send({
    ...
    async function updateSettingMain() {
        try {
            ...
            let maxPresaleLength = parseInt($('#max-presale-length').val() || 0);
            let minLiquidityPercent = parseFloat($('#min-liquidity-percent').val() || 0);
            let minLockPeriod = parseInt($('#min-lock-period').val() || 0);
            let maxSuccessToLiquidity = parseInt($('#max-success-to-liquidity').val() || 0);
            ...
            minLiquidityPercent = parseInt(minLiquidityPercent * 10);

            let presaleSettingAddress = $('#presale-setting-address').text();
            let presaleSettingContract = new web3.eth.Contract(ABI_PRESALE_SETTING, presaleSettingAddress);
            let result = await presaleSettingContract.methods.setSettingInfo(
                baseFeePercent,
                tokenFeePercent,
                creationFee,
                firstRoundLength,
    ...
    async function updateSettingAddress() {
        try {
            ...
            let dexLockerAddress = $('#dex-locker-address').val().trim();
            if (!web3.utils.isAddress(dexLockerAddress)) {
                toastr["error"]("Invalid dex locker address");
                return;
            }

            let presaleSettingAddress = $('#presale-setting-address').text();
            let presaleSettingContract = new web3.eth.Contract(ABI_PRESALE_SETTING, presaleSettingAddress);
            let result = await presaleSettingContract.methods.setSettingAddress(
                baseFeeAddress,
                tokenFeeAddress,
                adminAddress,
                wrapTokenAddress,
    ...
    async function deleteWhitelistToken(whitelistTokenAddress) {
        try {
            ...
            let presaleSettingAddress = $('#presale-setting-address').text();
            let presaleSettingContract = new web3.eth.Contract(ABI_PRESALE_SETTING, presaleSettingAddress);
            let result = await presaleSettingContract.methods.updateWhitelistToken(
    ...
    async function setZeroRound() {
        try {
            ...
            let presaleSettingAddress = $('#presale-setting-address').text();
            let presaleSettingContract = new web3.eth.Contract(ABI_PRESALE_SETTING, presaleSettingAddress);
            let result = await presaleSettingContract.methods.setZeroRound(
